;****************** main.s ***************
; Program written by: Evan Varghese and Ken Nguyen
...
DataBuf SPACE 100 ; uint8_t DataBuf[100];
TimeBuf SPACE 400 ; uint32_t TimeBuf[100];
	
DataPt	SPACE 4	; pointer to DataBuf array elements
TimePt	SPACE 4	; pointer to TimeBuf array elements
	
lTime	SPACE 4 ; the last time read by the debug cycle. initialize to 0x00FFFFFF
...
Debug_Init 
      PUSH 	{R0-R4,LR}					;AAPCS compliance 
	  
	  LDR  	R0, =DataBuf
	  MOV	R2, #-1
	  MOV	R3, #100
DataInitLoop	  
	  STRB	R2, [R0]					;overwrites every value in DataBuf with -1
	  ADD	R0, #1
	  SUBS	R3, #1
	  BNE	DataInitLoop
	  
	  LDR	R1, =TimeBuf			;does the same to TimeBuf
	  MOV	R3, #100
TimeInitLoop
	  STR	R2, [R1]
	  ADD	R1, #4					;since this array is 4 bit, the pointer must be increased by 4
	  SUBS	R3, #1
	  BNE	TimeInitLoop
	  
	  LDR	R0, =DataBuf			;stores the initial pointers to the proper variables
	  LDR	R1, =DataPt
	  STR	R0, [R1]
	  LDR	R0, =TimeBuf
	  LDR	R1, =TimePt
      ...
      POP {R0-R4,PC}					;popping to LR to PC returns to the original main.s code
	  
	  
;Debug capture      
Debug_Capture 
	  PUSH {R0-R6,LR}
	  LDR	R3, =DataPt					;check if buffers are full
	  LDR	R3, [R3]
	  LDR	R4, =DataBuf
	  ADD	R4, #100
	  CMP	R3, R4
	  ...
	  LDR	R5, =lTime
	  LDR	R5, [R5]
	  SUB	R6, R5, R1					;subtract current time from last time since it counts down
	  AND	R6, #0X00FFFFFF
	  LDR	R5, =lTime
	  STR	R1, [R5]					;make last time the current time
	  
	  LDR	R0, =TimePt					;store the time into TimePt
	  LDR	R0, [R0]
	  STR	R6, [R0]
	  
	  LDR	R1, =TimePt
	  ADD	R0, #0x4					;increment TimePt
	  STR	R0, [R1]
	  
	  
	  
	
	  LDR	R3, =GPIO_PORTA_DATA_R		;check ports A and E
	  ...
	  AND	R3, #0x10
	  AND	R4, #0xF
	  ORR	R2, R3, R4					;R2 has the masked and combined values of everything
		
	  LDR	R3, =DataPt					;store that value into DataBuf
	  LDR	R3, [R3]					;take the address in DataPt
	  STRB	R2, [R3]
	  
	  LDR	R4, =DataPt
	  ADD	R3, #0x1					;increment DataPt
	  STR	R3, [R4]					;this all takes 825 ns. the full program takes about 80 million nanoseconds to run, so the debug routine has an intrusiveness of .001%
